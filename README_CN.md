...
检索评测（`evals/`）把夹具仓库跑过真实的扫描与索引链路，检查已知答案所在的文件还能不能被检索回来，普通通道和模块卡片通道都查（卡片通道的夹具里专门有换说法和中文提问）。它在 pytest 里是硬门槛，在 CI 里是软门槛（标红但不阻塞）；有意改动检索行为后跑 `python evals/run_eval.py --update-baseline` 记录新基线。
